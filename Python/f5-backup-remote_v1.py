...
def credential():
    ...
    user = opts.f5username
    pwd1 = opts.f5password
    # start infinite loop
    return user, pwd1
    #while True:
...
def create_ucs(session):
    ...
    f5host = 'https://%s/mgmt/tm/sys/global-settings?$select=hostname' % HOSTNAME
    fwhost = session.get(f5host).json()
    #api_response_dict = json.loads(fwhost)
    settings = fwhost['hostname'].split('.')[0]
    # print(settings)
    # generate a timestamp file name
    ucs_filename = settings + '_' + HOSTNAME + '_' + datetime.datetime.now().strftime('%Y-%m-%d-%H%M%S') + '.ucs'
...
if __name__ == "__main__":
    ...
    parser.add_option('--f5username', help='Pass the F5 Big-ip username')
    parser.add_option('--f5password', help='Pass the F5 Big-ip password')
    ...
    if not opts.hostname:
        ...
        exit(1)
    elif not opts.f5username:
        print('--f5username argument is required.')
        exit(1)
    elif not opts.f5password:
        print('--f5password argument is required.')
        exit(1)
